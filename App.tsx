...
export default function App() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } catch (e) { console.error('Local IP error:', e); }

      // Get or Prompt Device Name
      let deviceName = localStorage.getItem('deviceName');
      if (!deviceName) {
        deviceName = window.prompt('Enter Device Name (e.g., USER-PC):') || 'Unknown-PC';
                ...
                <form onSubmit={handleTaskSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="max-h-[200px] overflow-y-auto custom-scrollbar p-1">
                          {models.filter(m => {
                            if (user?.role === 'MODEL_MANAGER') {
                              ...
                              if (myEmpId === '37091') return ['18K', 'Others Work'].includes(m);
                  ...
                  <div>
                    ...
                    >
                      {urgencies.filter(u => {
                        if (u === 'MOST_URGENT') {
                          return user?.role === 'ENGINEER' || user?.role === 'SUPER_ADMIN' || user?.role === 'HOD' || user?.role === 'IN_CHARGE' || user?.role === 'MODEL_MANAGER' || user?.employeeId === '42949';
                        }
                        return true;
